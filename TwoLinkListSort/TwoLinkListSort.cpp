...
ListObj* split(ListObj* head);

// Функция слияния
ListObj* merge(ListObj* first, ListObj* second)
{
    // Если первый полученый список пустой
    if (!first)
        return second;

    // Если второй полученый список пустой
    if (!second)
        return first;

    // Находим меньшее значение и связываем элементы списка между собой
...
ListObj* mergeSort(ListObj* head)
{
    // Проверяем существует ли список, и ккол-во элементов в нем > 1
    if (!head || !head->next)
        return head;
    // Получаем вторую половину списка
    ListObj* second = split(head);

    // Рекурсия для правой и левой части
    // Заново пытаемся разделить и отсортировать списки
    head = mergeSort(head);
    second = mergeSort(second);

    // Сливаем две отсортированные части списка в одну
...
ListObj* split(ListObj* head)
{
    // Создаем два указателя. Первый перещемается по списку быстро, второй медленно
    ListObj* fast = head, * slow = head;
    ...
    // Запоминаем середину списка
    ListObj* temp = slow->next;
    slow->next = NULL;
    // Отправляем
    return temp;
}
...
int main()
{
    ...
    do
    {
        ...
        if (menuChoice == 3) show();
        if (menuChoice == 4) del();
        if (menuChoice == 5) safe();
        if (menuChoice == 6) load();
        //if (menuChoice == 8) replace();
        if (menuChoice == 7) head = mergeSort(head);
